...
Statly 是一个常驻 macOS 菜单栏的系统监控工具。CPU、内存、温度、网速、磁盘五项指标直接显示在菜单栏上，点哪一项就展开哪一项的详情。

它的设计目标只有一个：**在不被察觉的前提下把信息递到眼前**。监控工具本身不该成为耗电大户，所以整个 App 单进程、不到 1 MB，没有后台守护进程、没有权限弹窗、没有第三方依赖。

![状态栏与详情弹窗](docs/statusbar.jpg)

## 功能特点

**五个模块，各自独立** — 每个模块占一个菜单栏图标，可单独开关、按住 ⌘ 拖动排序，位置由系统记住。刘海屏放不下就关掉几个。

**一眼可读** — 占用类指标用圆环加百分比呈现，圆环角度与读数严格对应；网速为上下行双行速率。数字用等宽字体并按最宽情形定宽，数值跳动不会让整条菜单栏左右晃动。

**样式可搭配** — 占用样式（圆环+百分比 / 纯文本）× 标签样式（图标 / 竖排 / 横排文本 / 隐藏），统一应用到所有模块，在设置里即时预览。

**详情不止于数字** — 点击图标展开该模块的历史曲线；CPU 与内存还会列出占用最高的 5 个进程，磁盘显示容量用量条与读写速率，内存显示压力状态与 App / 联动 / 已压缩明细。

**悬停即精确值** — 圆环模式下想看小数或完整速率，把鼠标停在图标上即可，不必点开。

**原生外观** — 图标为模板图，自动适配深浅色菜单栏；设置窗口用系统的分组卡片与毛玻璃材质；全中文界面。

**省电是实现出来的，不是说出来的** — 所有模块共享一次定时唤醒；渲染结果与上次相同就完全不碰菜单栏；锁屏或显示器休眠时暂停采样；进程排行只在弹窗打开期间运行。

**干净** — 无 Dock 图标、无登录项守护进程、不申请任何系统权限、除手动检查更新外不联网。卸载就是删掉 App 加一个配置文件。
...
侧边栏按模块分区。样式改动在「预览」里即时可见，不用关掉窗口去菜单栏比对。刷新间隔可选 1 / 2 / 5 秒，默认 2 秒——这是显示效果与功耗的平衡点。开机自启走系统的 `SMAppService`，不装任何登录项守护进程。

![设置窗口](docs/settings.jpg)

## 性能

轻量是这个项目的第一原则。以下都是实测值（Apple Silicon，五模块全开，2 秒刷新）：
...
- **单 timer 合并唤醒**：所有模块共享一个 `DispatchSourceTimer`，一次唤醒采完全部指标。唤醒次数比每次唤醒干多少活更影响耗电。
- **变了才画**：格式化结果与上次相同就完全跳过 `NSStatusItem` 调用。
- **看不见就停**：锁屏、显示器休眠时暂停采样，唤醒后重置基线以免出现速率尖峰。
- **按需分级**：进程排行只在弹窗打开期间采样，关闭即停止并清空状态。
- **温度独立节流**：温度是慢变量，固定 5 秒采一次，且只读 4 个传感器——实测取 4 个的最高值与全量读取平均只差 0.01°C，成本从 16.7 ms 降到 3.6 ms。

CPU 一项目前超出目标，瓶颈在菜单栏图像的离屏绘制（开销随刷新频率与模块数线性增长），计划用渲染结果缓存解决。
...
`project.yml` 是 Xcode 工程的唯一事实来源（生成的 `.xcodeproj` 不入库）；`Package.swift` 负责快速开发运行。

```
Sources/StatlyKit/
├── App/        入口、AppCoordinator（采样循环、菜单栏、弹窗、设置的总协调）
...
- ✅ 五个模块全链路：采样 → 菜单栏 → 详情弹窗 → 设置
- ✅ 进程排行、检查更新、DMG 发布流程
- ⏳ 菜单栏渲染缓存（把 CPU 拉回 0.3% 目标以内）
- ⏳ Developer ID 签名与公证
